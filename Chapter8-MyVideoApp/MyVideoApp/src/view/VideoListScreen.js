...
export default class VideoListScreen extends React.Component {
  ...
  static navigationOptions = ({ navigation }) => ({
    headerTitle: (navigation.state.routeName === 'VideoList') ? '列表模式' : '全屏模式',
  });
  ...
  _renderRow = (item) => {
    let url = item.item;
    return (
      <TouchableOpacity onPress={() => { this.itemSelected(url) }}>
  ...
  itemSelected(url) {
    if (this.props.navigation.state.routeName === 'VideoList') {
